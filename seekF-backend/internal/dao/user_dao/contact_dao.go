...
func (d *ContactDAOImpl) RemoveContact(userId string, contactId string) error {
	result := d.db.Where("user_id = ? AND contact_id = ?", userId, contactId).Delete(&models.UserContact{})
	return result.Error
}

// RemoveContactsByContactId 批量删除指定联系ID的联系人
func (d *ContactDAOImpl) RemoveContactsByContactId(contactId string) error {
	result := d.db.Where("contact_id = ?", contactId).Delete(&models.UserContact{})
	return result.Error
}
...
func (d *ContactDAOImpl) UpdateUserContactStatusAndDelete(userId string, contactId string, status int8) error {
	...
	result := d.db.Model(&models.UserContact{}).Where("user_id = ? AND contact_id = ?", userId, contactId).Updates(map[string]interface{}{
...
func (d *ContactDAOImpl) GetUserJoinedGroupContactsByUserId(userId string) ([]models.UserContact, error) {
	...
	result := d.db.Order("created_at DESC").Where("user_id = ? AND status != ? AND status != ?", userId, contactstatusenum.QUIT_GROUP, contactstatusenum.KICK_OUT_GROUP).Find(&contactList)
...
func (d *ContactDAOImpl) GetUserContactList(ownerId string) ([]models.UserContact, error) {
	var contactList []models.UserContact
	result := d.db.Order("created_at DESC").Where("user_id = ? AND status != ?", ownerId, contactstatusenum.DELETE).Find(&contactList)
	return contactList, result.Error
}
